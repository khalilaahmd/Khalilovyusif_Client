...
function Bouquets () {
    ...
    const deleteImage = (bouquetsId) => {
        axios
           .delete(`${API_URL}/api/bouquets/${bouquetsId}`)
           .then(() => {
            const updatedImages = bouquets.filter(
                (image) => image._id !== bouquetsId
            );
            setBouquets(updatedImages);
           })
           .catch((error) => console.log(error))
    };
